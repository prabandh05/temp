    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            for sport_name, sport in sports.items():
                ...
                self.stdout.write(self.style.SUCCESS(f"✓ Coach: {coach_user.username} (ID: {coach.coach_id})"))

            # 3. Create 30 Cricket Players (NOT linked to coaches - to show invitation flow)
            cricket_players = []
            ...
            for i in range(1, 31):
                ...
                # Create sport profile but DON'T link to coach (to show invitation flow)
                profile, _ = PlayerSportProfile.objects.get_or_create(
                    player=player,
                    sport=cricket_sport,
                    defaults={
                        "coach": None,  # Not linked initially
                        "team": None,
                        "is_active": False,
                        "career_score": 0.0,
                    }
                ...
                self.stdout.write(self.style.SUCCESS(f"✓ Cricket Player {i}: {player_user.username}"))

            # 4. Create players for other sports and link them to coaches as students
            other_sports_config = [
        ...
        summary = (
            ...
            + f"Cricket Players: 30 (NOT linked to coaches - ready for invitation flow)\n"
            ...
            + "\nNotes:\n"
            + "- Cricket players are NOT linked to coaches (to demonstrate invitation flow)\n"
            + "- Other sports players are auto-linked to coaches as students\n"
            + "- No sessions, teams, tournaments, or managers created\n"
